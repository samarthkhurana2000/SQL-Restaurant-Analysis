...
create view  average_analysis_ratings as(
select a.name_,
...
create view best_restaurant as(
select 
first_value(cuisine) over(partition by cuisine order by overall_rating) as best_for_overall,
first_value(cuisine) over(partition by cuisine order by food_rating) as best_for_food,
first_value(cuisine) over(partition by cuisine order by service_rating) as best_for_service
from average_analysis_ratings);


-- select *
-- from best_restaurant
-- group by best_for_overall,best_for_food,best_for_service
-- order by cuisine;
...
-- 40 total customer with highet rating in all categories of rating
